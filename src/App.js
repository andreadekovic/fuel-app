...
const defaultProjects = [
  { id:1, initials:"BC", name:"Brand Campaign", members:4, balance:6200, pct:67, split:"40/30/30", accent:"#F5E642" },
  { id:2, initials:"DS", name:"Dev Sprint Q2",  members:2, balance:3750, pct:42, split:"60/40",    accent:"#4B4BD4" },
  { id:3, initials:"RF", name:"Research Fund",  members:3, balance:2530, pct:25, split:"Equal",    accent:"#2B8EF0" },
];

const activity = [
  { type:"in",    label:"Client payment · Brand",    date:"Today, 2:14 PM", amount:"+$5,000", color:"#F5E642" },
  { type:"split", label:"Auto split triggered",      date:"Today, 2:14 PM", amount:"–$5,000", color:"#2B8EF0" },
  { type:"out",   label:"Payout · Ana N.",           date:"Yesterday",      amount:"–$1,500", color:"#4B4BD4" },
  { type:"in",    label:"Invoice paid · Dev Sprint", date:"Apr 11",         amount:"+$3,200", color:"#F5E642" },
];
  ...
  return (
    <div style={{ background:C.sidebar, borderRight:`1px solid ${C.border}`, padding:"24px 0", display:"flex", flexDirection:"column", fontFamily:fonts, height:"100vh", position:"sticky", top:0 }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"0 20px 32px" }}>
        <div style={{ width:32, height:32, borderRadius:8, background:`linear-gradient(135deg, ${C.orange}, ${C.pink})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, fontWeight:800, color:C.light, fontFamily:titleFonts }}>F</div>
        <span style={{ fontSize:18, fontWeight:700, color:C.light, fontFamily:titleFonts }}>fuel</span>
      </div>
      ...
      {nav.map(item => (
        <div key={item} onClick={() => setPage(item)}
          style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 20px", fontSize:13, cursor:"pointer",
            ...
            background: page===item ? `${C.yellow}12` : "transparent",
            color: page===item ? C.yellow : C.muted }}>
      ...
      {projects.map(p => (
        <div key={p.name} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 20px", fontSize:13, color:C.muted, cursor:"pointer" }}>
    ...
    <div style={{ padding:32, display:"flex", flexDirection:"column", gap:24, fontFamily:fonts }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
        <div>
          ...
          <h1 style={{ fontSize:32, fontWeight:700, color:C.light, margin:0, fontFamily:titleFonts }}>Dashboard</h1>
        </div>
        <div style={{ display:"flex", gap:10 }}>
          <button style={{ background:"transparent", border:`1px solid ${C.border}`, borderRadius:8, padding:"9px 16px", fontSize:13, color:C.muted, cursor:"pointer", fontFamily:fonts }}>Receive</button>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
        {metrics.map(m => (
          <div key={m.label} style={{ background:m.accent ? C.yellow : C.card, border:`1px solid ${m.accent ? C.yellow : C.border}`, borderRadius:12, padding:18 }}>
            <div style={{ fontSize:11, color:m.accent ? "rgba(15,11,42,.6)" : C.muted, textTransform:"uppercase", letterSpacing:".06em", marginBottom:8 }}>{m.label}</div>
            <div style={{ fontSize:28, fontWeight:700, color:m.accent ? C.bg : C.light, lineHeight:1, fontFamily:titleFonts }}>{m.value}</div>
            <div style={{ fontSize:11, color:m.accent ? "rgba(15,11,42,.5)" : m.subYellow ? C.yellow : C.dim, marginTop:6 }}>{m.sub}</div>
          </div>
        ))}
      </div>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:20 }}>
          ...
          {projects.map(p => (
            <div key={p.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 0", borderBottom:`1px solid ${C.inner}` }}>
              <div style={{ width:38, height:38, borderRadius:10, background:`${p.accent}22`, color:p.accent, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, flexShrink:0 }}>{p.initials}</div>
              <div style={{ flex:1 }}>
                ...
                <div style={{ height:2, background:C.inner, borderRadius:2, marginTop:7, width:100 }}>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:20 }}>
          ...
          {activity.map((tx,i) => (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:i<activity.length-1 ? `1px solid ${C.inner}`:"none" }}>
              <div style={{ width:32, height:32, borderRadius:"50%", background:`${tx.color}22`, color:tx.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, flexShrink:0 }}>
                {tx.type==="in" ? "↓" : tx.type==="out" ? "↑" : "⋮"}
              </div>
              <div>
                <div style={{ fontSize:12, color:C.light }}>{tx.label}</div>
  ...
  return (
    <div style={{ padding:32, fontFamily:fonts }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:24 }}>
        <h1 style={{ fontSize:32, fontWeight:700, color:C.light, margin:0, fontFamily:titleFonts }}>Projects</h1>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14 }}>
        {projects.map(p => (
          <div key={p.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:18, cursor:"pointer" }}>
            <div style={{ width:42, height:42, borderRadius:10, background:`${p.accent}22`, color:p.accent, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700, marginBottom:14 }}>{p.initials}</div>
            ...
            <div style={{ fontSize:22, fontWeight:700, color:C.light, marginBottom:6, fontFamily:titleFonts }}>${p.balance.toLocaleString()}</div>
            <div style={{ height:2, background:C.inner, borderRadius:2, marginBottom:12 }}>
              <div style={{ height:"100%", width:`${p.pct}%`, background:p.accent, borderRadius:2 }} />
            </div>
  ...
  return (
    <div style={{ padding:32, fontFamily:fonts }}>
      <div style={{ marginBottom:24 }}>
        ...
        <h1 style={{ fontSize:32, fontWeight:700, color:C.light, margin:0, fontFamily:titleFonts }}>Send funds</h1>
      </div>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:20 }}>
          ...
          {recipients.map((r,i) => (
            <div key={i} onClick={() => setSelected(i)}
              style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 14px", border:`1px solid ${selected===i ? r.color : C.border}`, borderRadius:10, marginBottom:8, cursor:"pointer", background:selected===i ? `${r.color}12` : "transparent" }}>
